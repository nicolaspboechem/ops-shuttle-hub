...
function MotoristaCardReadOnly({ motorista }: { motorista: MotoristaComVeiculo }) {
...
function LocationGroup({ title, icon: Icon, iconColor, motoristas, defaultOpen = true }: LocationGroupProps) {
    ...
    <Collapsible open={isOpen} onOpenChange={setIsOpen}>
      ...
      <CollapsibleContent>
        <div className="space-y-2 pl-6 pb-4">
          {motoristas.map(m => (
            <MotoristaCardReadOnly key={m.id} motorista={m} />
          ))}
        </div>
      </CollapsibleContent>
    </Collapsible>
...
export function ClienteLocalizadorTab({ eventoId }: ClienteLocalizadorTabProps) {
  const { motoristas, motoristasPorLocalizacao, loading } = useLocalizadorMotoristas(eventoId);
  ...
  const naBase = activeMotoristas.filter(m => 
    m.ultima_localizacao === baseName || m.ultima_localizacao === 'Base'
  );
  ...
  const filteredMotoristas = useMemo(() => {
    ...
    if (locationFilter === 'em_transito') filtered = emTransito;
    else if (locationFilter === 'base') filtered = naBase;
    else if (locationFilter === 'outros') filtered = [...outrosLocais, ...semLocal];
