...
export function OperadorHistoricoTab({ viagens }: OperadorHistoricoTabProps) {
  ...
  const totalPaxIda = viagensFinalizadas
    .filter(v => v.status === 'encerrado')
    .reduce((sum, v) => sum + (v.qtd_pax || 0), 0);

  const totalPaxVolta = viagensFinalizadas
    .filter(v => v.status === 'encerrado')
    .reduce((sum, v) => sum + (v.qtd_pax_retorno || 0), 0);
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-2">
        <div className="text-center p-3 rounded-lg bg-primary/10">
          ...
          <p className="text-xl font-bold text-primary">{totalPaxIda}</p>
          <p className="text-[10px] text-muted-foreground">PAX Ida</p>
        </div>
        <div className="text-center p-3 rounded-lg bg-primary/10">
          <div className="flex items-center justify-center gap-1.5 mb-1">
            <ArrowDown className="h-3.5 w-3.5 text-primary" />
          </div>
          <p className="text-xl font-bold text-primary">{totalPaxVolta}</p>
          <p className="text-[10px] text-muted-foreground">PAX Volta</p>
        </div>
      </div>
